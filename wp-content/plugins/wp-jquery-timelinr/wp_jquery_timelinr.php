...
if (!class_exists('jqueryTimelinrLoad')) {
	...
	class jqueryTimelinrLoad extends Broobe_TL_Plugin_Admin {
		...
		private function loadConstants() {
			define('JQTL_CURRENT_VERSION', '1.0');
		...
		public function timelinr_activated(){
			...
			*/
	        ...
	        if (!isset($options['arrowkeys']))
	            $options['arrowkeys'] = 'false';
	               
	        if (!isset($options['autoplay']))
	            $options['autoplay'] = 'no';
	        
	        if (!isset($options['autoplaydirection']))
	            $options['autoplaydirection'] = 'backward';
	        
			if (!isset($options['autoplaypause']))
	            $options['autoplaypause'] = '2000';
		...
		public function adminActions() {
			...
			function timelinr_meta_options(){    
				...
				<script type="text/javascript">
					jQuery(document).ready(function($) {
						var queryDate = '<?php echo $custom["timelineDate"][0];?>';
			...
			function broobe_date_picker() {			
				...
				//wp_localize_script( 'custom.timelinr', 'MyAjax', array( 'ajaxurl' => admin_url( 'admin-ajax.php' ) ) ); 
			}
		...
		function timelinr_plugin_action_links($links) {
		...
		function timelinr_plugin_meta_links( $links, $file ) {
		...
		public function shortcode($atts, $content = NULL) {
			...
			$pairs = array(
					...
					'startat' => intval($options['startat']),
					'arrowkeys' => $options['arrowkeys'],
					'autoplay' => $options['autoplay'],
					'autoplaydirection' => $options['autoplaydirection'],
					'autoplaypause' => intval($options['autoplaypause']),
					'order' => $options['order'],
			...
			if ( $atts['orientation'] == 'horizontal' ){
